...
app.post('/patients/admin', async (req, res) => {
  ...
  try {
    ...
    // 2) Insere na fila com status = 'waiting'
    const [qResult] = await conn.query(
      `INSERT INTO queue_entries (patient_id, is_priority, status, created_at)
       VALUES (?, false, 'waiting', NOW())`,
      [patientId]
    );
    const queueEntryId = qResult.insertId;

    await conn.commit();

    // 3) Busca dados recém-criados (para retorno ao front)
    const [[patient]] = await conn.query('SELECT * FROM patients WHERE id = ?', [patientId]);
    const [[queueEntry]] = await conn.query('SELECT * FROM queue_entries WHERE id = ?', [queueEntryId]);

    res.status(201).json({ patient, queueEntry });
  } catch (err) {
    await conn.rollback();
    console.error('Erro ao cadastrar paciente/Admin:', err);
    res.status(500).json({ error: err.message });
  } finally {
    conn.release();
  }
